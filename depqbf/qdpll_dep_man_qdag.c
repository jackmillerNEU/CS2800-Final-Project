...
/* 'print_deps' function. */
static void
qdpll_dep_man_return_deps_by_search (QDPLLDepManGeneric * dmg, VarID id, unsigned int* ret_deps)
{
  QDPLLDepManQDAG *dm = (QDPLLDepManQDAG *) dmg;
  assert (id > 0 && id < dm->pcnf->size_vars);
  assert (dm->state.init);
  ...
  Var **p, **e;
  unsigned pos_i;
  for (p = deps.start, e = deps.top, pos_i = 0; p < e; p++, pos_i++)
  {
    Var *d = *p;
    // fprintf (stdout, "%u ", d->id);
    ret_deps[pos_i] = d->id;
                           ...
                           QDPLL * qdpll)
{
  ...
  dm->dmg.return_deps = qdpll_dep_man_return_deps_by_search;
